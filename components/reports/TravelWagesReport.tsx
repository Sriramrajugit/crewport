...
const TravelWagesReport = forwardRef<{ handleExportExcel: () => void }, Props>(
    ({ month, year }, ref) => {
        ...
        const fetchReportData = async () => {
            ...
            try {
                const response = await fetch(
                    `/api/reports/travel-wages?vesselId=${selectedVessel.vessel_id}&month=${month}&year=${year}`,
                    {
                        headers: {
        ...
        useImperativeHandle(ref, () => ({
            handleExportExcel: () => {
                try {
                    ...
                    // Create worksheet
                    const headers = ['Employee Name', 'Rank', 'Date', 'No. of Days', 'Amount'];
                    ...
                    XLSX.utils.book_append_sheet(workbook, ws, 'Travel Wages');
        ...
        if (entries.length === 0) {
            return (
                <div className="bg-yellow-50 border border-yellow-200 p-6 rounded-lg text-center">
                    <p className="text-gray-600">No Travel Wages entries found for this month</p>
                </div>
            );
        }
            ...
            <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        ...
                        <p className="text-2xl font-bold text-green-600">{summary.total_entries}</p>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        <p className="text-sm text-gray-600">Total Days</p>
                        <p className="text-2xl font-bold text-green-600">{summary.total_days}</p>
                    </div>
                    <div className="bg-green-50 p-4 rounded-lg border border-green-200">
                        <p className="text-sm text-gray-600">Total Amount</p>
                        <p className="text-2xl font-bold text-green-600">${summary.total_amount.toFixed(2)}</p>
                    </div>
                </div>
                            ...
                            <tbody className="divide-y divide-gray-200">
                                {entries.map((entry, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-6 py-3 text-sm text-gray-600">{new Date(entry.date).toLocaleDateString()}</td>
                                ...
                                <tr className="bg-gray-100 font-bold border-t-2 border-gray-300">
                                    <td colSpan={3} className="px-6 py-3 text-right text-sm">TOTAL:</td>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-sm">
                                        <thead className="bg-gray-50 border-b border-gray-200">
                                            <tr>
                                                <th className="px-6 py-2 text-left text-xs font-semibold text-gray-700">Date</th>
                                        ...
                                        <tbody className="divide-y divide-gray-200">
                                            {crewData.entries.map((entry: ReportEntry, idx: number) => (
                                                <tr key={idx} className="hover:bg-gray-50">
                                                    <td className="px-6 py-2 text-gray-600">{new Date(entry.date).toLocaleDateString()}</td>
...
export default TravelWagesReport;
