...
"[project]/app/api/slopchest/summary/route.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
...
async function GET(request) {
    return (0, __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$accessControl$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["withVesselAccess"])(request, async (vesselId, userId)=>{
        try {
            ...
            const allSummaryData = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].inventoryConsumption.findMany({
                where: {
                    vessel_id: vesselId,
            ...
            const slopchestData = allSummaryData.filter((c)=>c.inventory_items?.inventory_type === 'SLOPCHEST');
            // Group by crew_member_id
            const summaryDataGrouped = slopchestData.reduce((acc, item)=>{
                const totalDedNum = parseFloat(item.total_deduction.toString());
                const existing = acc.find((s)=>s.crew_member_id === item.crew_member_id);
                if (existing) {
            ...
            const summary = crewMembers.map((crew)=>{
                const deductionData = summaryDataGrouped.find((s)=>s.crew_member_id === crew.id);
                return {
            ...
            const crewTotal = summary.reduce((sum, crew)=>sum + parseFloat(crew.total_deduction.toString()), 0);
            // Get on-signers total (filter for SLOPCHEST)
            const allOnSigners = await __TURBOPACK__imported__module__$5b$project$5d2f$lib$2f$prisma$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["prisma"].inventoryOnSigner.findMany({
                where: {
                    vessel_id: vesselId,
                    month: parseInt(month),
                    year: parseInt(year)
            ...
            const slopchestOnSigners = allOnSigners.filter((s)=>s.inventory_items?.inventory_type === 'SLOPCHEST');
            const onSignersTotal = slopchestOnSigners.reduce((sum, s)=>sum + parseFloat(s.total_deduction.toString()), 0);
            return __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$next$2f$server$2e$js__$5b$app$2d$route$5d$__$28$ecmascript$29$__["NextResponse"].json({
                month: parseInt(month),
                year: parseInt(year),
...
//# sourceMappingURL=%5Broot-of-the-server%5D__c1f1d78c._.js.map